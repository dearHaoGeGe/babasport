	...
	<div class="body-box">
		<form action="/brand/list.do" method="post" style="padding-top: 5px;">
			品牌名称: <input type="text" name="name" value="${name}" /> <select
				name="isDispaly">
				<option value="1"
					<c:if test="${isDispaly == 1}">selected="selected"</c:if>>是</option>
				<option value="0"
					<c:if test="${isDispaly == 0}">selected="selected"</c:if>>否</option>
			...
			<tbody class="pn-ltbody">
				<c:forEach items="${pagination.list}" var="brand">
					<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
						...
						<td align="center"><a class="pn-opt" href="/brand/toEdit.do?id=${brand.id}">修改</a> | <a
