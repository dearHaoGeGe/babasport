...
<script type="text/javascript">
	//全选
	function checkBox(names, checked) {
		//选择name=ids的input，设置checked属性，传进来是什么就是什么
		$("input[name='ids']").attr("checked", checked);
	}

	//删除
	function optDelete(name, isDisplay, pageNo) {
		var size = $("input[name='ids']:checked").size(); //选择name=ids的input中的选中的
		if (size == 0) {
			alert("请至少选择一个");
			return;
		}
		...
		//jQuery提交
		var jvF = $("#jvForm");
		jvF.attr("action", "/brand/deletes.do?name=" + name + "&isDisplay="
				+ isDisplay + "&pageNo=" + pageNo);
		jvF.attr("method", "post");
		jvF.submit();
	}
</script>
</head>
<body>
	...
	<div class="body-box">
		<form action="/brand/list.do" method="post" style="padding-top: 5px;">
			...
			<select name="isDispaly">
				<option value="1" <c:if test="${isDispaly == 1}">selected="selected"</c:if>>是</option>
				<option value="0" <c:if test="${isDispaly == 0}">selected="selected"</c:if>>否 </option>
			</select>
			...
			<tbody class="pn-ltbody">
				<c:forEach items="${pagination.list}" var="brand">
					<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'"
						...
						<td align="center">
							<a class="pn-opt" href="/brand/toEdit.do?id=${brand.id}">修改</a> | <a
